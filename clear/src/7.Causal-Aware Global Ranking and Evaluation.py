...
RANKING_EVALUATION_RESULT_DIRECTORIES = {
    'confidence_enhanced': 'ranking_evaluation',
    'suspiciousness_only': (
...
def build_ranking_strategies() -> Tuple[RankingStrategy, ...]:
    ...
    return (
        RankingStrategy(
            name=(
                'global_file_context_then_line_causal_'
                'then_suspiciousness'
            ),
            sort_columns=(
...
def add_counterfactual_features(
    ...
    output_dataframe[
        'contains_any_counterfactual_token'
    ] = (
            (
                    output_dataframe[
                        'contains_line_counterfactual_token'
...
def rank_indices_by_columns(
    ...
    effective_columns = ['predicted_file_bug']
    effective_columns.extend(sort_columns)
...
def save_clear_method_metric_csv(
    ...
    print(
        f'[{token_score_type}][{aggregation_method}] '
        f'Saved CLEAR metric table to {output_path}'
    )

    return output_path


def save_predefined_method_results(
        dataset_name: str,
        aggregation_method: str,
        predefined_strategy: RankingStrategy,
    ...
    for train_release, test_release in release_pairs:
        ...
        evaluation_rows.append(
            build_evaluation_row(
                ...
                test_release=test_release,
                aggregation_method=aggregation_method,
                strategy_name=predefined_strategy.name,
                token_score_type=token_score_type,
                metrics=metrics,
            )
        )
        ...
        print(
            f'[{test_release}] Saved {aggregation_method} '
            f'ranking to {ranking_output_path}'
        )
        ...
        json.dump(
            {
                ...
                'aggregation_method': aggregation_method,
                'token_score_type': token_score_type,
                'ranking_strategy': predefined_strategy.name,
    ...
    print(
        f'[{token_score_type}][{aggregation_method}] '
        f'Saved final evaluation to {evaluation_output_path}'
    )
    print(
        f'[{token_score_type}][{aggregation_method}] '
        f'Saved metadata to {metadata_output_path}'
    )
...
def main() -> None:
    ...
    predefined_strategy = RANKING_STRATEGIES[0]
    ...
    for token_score_type in TOKEN_SCORE_TYPES_TO_EVALUATE:
        ...
        for dataset_name, projects in datasets_projects.items():
            ...
            for aggregation_method in CLEAR_LINE_SCORE_METHODS:
                evaluation_dataframe = save_predefined_method_results(
                    dataset_name=dataset_name,
                    aggregation_method=aggregation_method,
                ...
                method_summary_rows.append({
                    'aggregation_method': aggregation_method,
                    'token_score_type': token_score_type,
                    'selected_ranking_strategy': predefined_strategy.name,
            ...
            strategy_metadata_path = os.path.join(
                ...
                'predefined_ranking_strategy.json',
            )
            with open(
                ...
                json.dump(
                    {
                        ...
                        'sort_columns': [
                            'predicted_file_bug',
                            *predefined_strategy.sort_columns,
                        ],
                    },
                    strategy_file,
                    indent=2,
            ...
            method_summary_path = os.path.join(
                summary_output_directory,
                'predefined_strategy_per_method_summary.csv',
            )
            ...
            print(
                ...
                'Saved strategy metadata to '
                f'{strategy_metadata_path}'
            )
            print(
                f'[{dataset_name}][{token_score_type}] '
                'Saved method summary to '
